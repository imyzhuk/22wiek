...
export async function GET(request: NextRequest) {
  ...
  products.forEach(product => {
    if (product.producer) {
      productsInfo.producers[product.producer.id] = product.producer
    }
    if (productsInfo.discountTypes.size < 3) {
      product.discountTypes.forEach((value) => productsInfo.discountTypes.add(value));
    }
    if (productsInfo.types.size < 3) {
      productsInfo.types.add(product.refrigerator?.type!)
    }
    ...
    productsInfo.freezerSectionsCountOptions.add(Number(product.refrigerator?.freezerSectionsCount))
  })

  return NextResponse.json<GetRefrigeratorMutableFiltersResponseType>(
    {
      ...productsInfo,
      freezerSectionsCountOptions: Array.from(productsInfo.freezerSectionsCountOptions).sort((a, b) => a - b),
      discountTypes: Array.from(productsInfo.discountTypes),
      types: Array.from(productsInfo.types),
      producers: Object.values(productsInfo.producers),
    }
  );
  */
}
...
/*
  ...
  const refrigetarorsInfo = await prisma.product.aggregate({
    where: {
      refrigerator: {
        is: {
